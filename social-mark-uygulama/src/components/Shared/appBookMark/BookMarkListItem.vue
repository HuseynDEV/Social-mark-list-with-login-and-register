...
export default {
  ...
  methods: {
    likeItem () {
      // const likes = this.$store.getters._userLikes
      // console.log(likes)

      console.log(this.$store.getters._userLikes)

      const likes=[...this.$store.getters._userLikes,this.item.id ]

      this.$axios
        .patch(
          `http://localhost:3000/users/${this.$store.getters._currentUserId}`,
          { likes }
        )
        .then(like_response => {
      ...
      // this.$axios.post('http://localhost:3000/users', {likes:[this.item.id]}).then(Response=>{
      //   console.log(Response)
      // })
      // console.log(this.$store.getters._currentUserId)
    }
  },
